"""
manual_inspect.py - Manual image quality annotation tool.
...
Usage:
    python manual_inspect.py [--target 1000] [--db image_index.sqlite] [--config config.json]
"""
...
def fetch_image_label(db_path: Path, image_id: int) -> str:
    """Return the label for a specific image_id, or None if not labeled."""
    conn = sqlite3.connect(db_path)
    try:
        row = conn.execute(
            "SELECT label FROM inspected_images WHERE image_id = ?",
            (image_id,)
        ).fetchone()
...
def build_ui(rgb_image: np.ndarray, row: tuple, mode_name: str,
             ...
             target: int, current_label: str = None) -> np.ndarray:
    ...
    # Line 1: case / type / label
    img_id, _, case_name, mtype = row
    label_text = f"   Label: {current_label}" if current_label else ""
    cv2.putText(
        ui,
        f"Case: {case_name}   Type: {mtype}   ID: {img_id}{label_text}",
    ...
    # Line 5: per-type good/bad/total breakdown
    types_sorted = sorted(label_counts.keys())
    type_stats = []
    for t in types_sorted:
        lc = label_counts.get(t, {"good": 0, "bad": 0})
        ...
        bad = lc["bad"]
        total = good + bad
        short = t[:8]  # Shorter name to fit more types
        type_stats.append(f"{short}: good={good} bad={bad} total={total}")
    stats_text = "  |  ".join(type_stats) if type_stats else "No labels yet"
    cv2.putText(
        ui,
        stats_text,
        (10, y0 + 88), font, 0.38, (200, 200, 100), 1,
    )
...
def run_inspect(root_dir: Path, db_path: Path, target: int, max_upscale: int) -> None:
    ...
    while 0 <= current_idx < len(queue):
        ...
        label_counts = fetch_label_counts(db_path)
        current_label = fetch_image_label(db_path, image_id)
        ui = build_ui(rgb, row, mode_name, current_idx, len(queue), label_counts, target, current_label)
        ...
        if key in (ord("q"), ord("Q")):
            ...
            label_counts = fetch_label_counts(db_path)  # Refresh counts immediately
            current_idx += 1
